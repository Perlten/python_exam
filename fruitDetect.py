...
def load_labels():
    ...
    #setting train_filelist to all files in dataset
    train_filelist = glob.glob(TRAIN_DATASET)
    #setting train_label_list to all labels, e.g "apple" etc.
    train_label_list = [name.split("_")[0].split("/")[-1] for name in train_filelist]
    #return a unique list of objects, "apple", "banana", "coffee" etc. so it can be used to transform the labels to 1,0 etc
    #why not just do it insta? makes no sense
